...
public class ConversationManager {
    ...
    private final List<Conversation> conversasList = new ArrayList<>();

    public interface ConversasListUpdateCallback {
        void onListaAtualizada(List<Conversation> listaOrdenada);
    ...
    private void processarConversa(DataSnapshot chatSnapshot) {
        ...
        conversation.setLastMessage(ultimaMsg != null ? ultimaMsg : "");
        conversation.setLastMessageTimestamp(timestamp != null ? timestamp : 0);
        ...
        conversation.setUnreadCount(isUnread ? 1 : 0);
        ...
        usersRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot userSnapshot) {
                ...
                conversation.setOtherUserNick(userSnapshot.child("nick").getValue(String.class));
                ...
                conversation.setOtherUserStatus(daysSinceLogin < 10);

                if (conversation.getOtherUserNick() == null) {
                    conversation.setOtherUserNick("Usuário Desconhecido");
                }
                
                addOrUpdateConversa(conversation);
            }
    ...
    private void addOrUpdateConversa(Conversation novaConversation) {
        int index = -1;
        for (int i = 0; i < conversasList.size(); i++) {
            if (conversasList.get(i).getChatId().equals(novaConversation.getChatId())) {
                index = i;
                break;
            }
        }
        ...
        if (index != -1) {
            conversasList.set(index, novaConversation);
        } else {
            conversasList.add(novaConversation);
        }
    ...
    private void notificarView() {
        if (updateCallback != null) {
            ...
            List<Conversation> listaCopia = new ArrayList<>(conversasList);
